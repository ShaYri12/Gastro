...
export default function MessagesPage() {
  ...
  const messageClicked = (msg) => {
    setSelectedRestaurant(msg);
    filteredRestaurants = filteredRestaurants.map(
      (restaurant) => (restaurant.id = msg.id && restaurant.isUnread === false)
    );
  };
    ...
    switch (activeTab) {
      case "ACTIVE":
        return restaurants.filter(
          (restaurant) =>
            restaurant.status === "In Review" && "Accepted" && "Adjusted"
        );
      case "CONFIRMED":
        return restaurants.filter(
        ...
        <div className="lg:w-[30%] w-[280px] custom-scrollbar space-y-[12px] overflow-y-auto h-[87vh]">
          {filteredRestaurants.map((restaurant) => (
            <div
              ...
              onClick={() => messageClicked(restaurant)}
            >
